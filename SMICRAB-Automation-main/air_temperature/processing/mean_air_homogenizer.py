...
class MeanAirHomogenization(BaseHomogenization):

    def __init__(self, eobs_file: str, era5_file: str, variable_name: str = "mean_air_temperature"):
    ...
    def save_results(self, output_path: str):
        ...
        self.save_homogenized_netcdf(
            ...
            global_attributes={"title": "Homogenized Mean Air Temperature Data"},
            compress=True
        )
    ...
    def calculate_moving_variance(self, time_series, time_series1):
        """
        Computes the difference in rolling variance between two time series.

        Args:
            time_series (pd.Series or np.array): First time series.
            time_series1 (pd.Series or np.array): Second time series.

        Returns:
            pd.Series: Difference in moving variances (var(series1) - var(series2)).
        """
        ...
        if ts.isna().all():
            return pd.Series([np.nan] * len(ts))

        # Calculate rolling variances (center-aligned, min_periods=1 for partial windows)
        mv = ts.rolling(window=self.window_size, center=True, min_periods=1).var(ddof=1)
        mv1 = ts1.rolling(window=self.window_size, center=True, min_periods=1).var(ddof=1)
    ...
    def calculate_acf(self, time_series: np.ndarray) -> np.ndarray:
        """
        Calculate auto correlation function (ACF) for a time series.

        Parameters:
        - time_series: 1D array-like (list, pandas Series, numpy array)

        Returns:
        - ACF values for lags 1 to acf_lag_max (excludes Lag 0)
        """
        ...
        # Exclude Lag 0 (which is always 1.0)
        return acf_values[1:]

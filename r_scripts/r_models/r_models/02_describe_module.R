      ...
      tryCatch(
        {
          ...
          pars <- pars_list[[ii]]
          if(summary_stat=="standard_deviation" | summary_stat=="range" | summary_stat=="count.NAs" | summary_stat=="skewness" | summary_stat=="kurtosis"){
            pars$unit <- NULL
            pars$limiti <- NULL
          }
          df <- dataframes[[ii]]
...
#     save(
#       list = c(
# #        "dataframes", "name.endogenous", "name.covariates",
#         "summary_stats"
#       ),
